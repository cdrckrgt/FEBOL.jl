...
struct FOVb <: Sensor
    ...
    function FOVb(cone_width::Real, alpha::Float64)
...
# 0 means it is not
function observe(m::SearchDomain, s::FOVb, p::Pose)
    ...
    o = (rand() < prob_in_view) ? true : false
    return o
end
...
function O(s::FOVb, theta::LocTuple, p::Pose, o)
    ...
    ret_val = (o == true) ? prob_in_view : (1.0 - prob_in_view)

    return ret_val
end


get_prob(s::FOVb, theta::LocTuple, xp::Pose, o) = O(s, theta, xp, o)
export get_prob
